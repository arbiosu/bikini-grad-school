...
import { useCreativeRoleForm } from '@/hooks/useCreativeRoleForm';
import { Label } from '@/components/ui/label';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import type { Tables } from '@/lib/supabase/database/types';

interface CreativeRoleFormProps {
  mode?: 'create' | 'edit';
  editData?: {
...
export function CreativeRoleForm(props: CreativeRoleFormProps) {
  ...
  const {
    ...
    isSuccess,
    isError,
  } = useCreativeRoleForm({ mode, editData });

  return (
    <section id='form' className='mx-auto max-w-7xl px-4'>
      <h6 className='font-bold'>
        {mode === 'create' ? 'Create' : 'Edit'} Creative Role
      </h6>
        ...
        >
          <div>
            ...
            <Input
              ...
              disabled={isLoading}
              required
              placeholder='e.g., Photography, Author, Model'
            />
          </div>
