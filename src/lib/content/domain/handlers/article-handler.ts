// lib/content/domain/handlers/article-handler.ts

import { BaseHandler } from '@/lib/common/base-handler';
import { type ValidationResult, ArticleData } from '@/lib/content/domain/types';
...
export class ArticleHandler extends BaseHandler<ArticleData> {
  ...
  validateMarkdownStructure(body: string): string[] {
    ...
    if (codeBlockMatches && codeBlockMatches.length % 2 !== 0) {
      warnings.push('Unclosed code block detected in article body');
    }

    // Check for very long lines (potential formatting issue)
    const lines = body.split('\n');
    const veryLongLines = lines.filter((line) => line.length > 1000);
    if (veryLongLines.length > 0) {
      warnings.push(
        `${veryLongLines.length} very long line(s) detected (>1000 chars) - may cause formatting issues`
      );
    }
    ...
    if (openBrackets !== closeBrackets) {
      warnings.push(
        'Unbalanced square brackets detected - check markdown links'
      );
    }
